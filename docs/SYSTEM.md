...
| P-0008  | Caddy + Auth    | [IN PROGRESS] | Replace `web_proxy.py` with Caddy (TLS, forward_auth); TOTP login (no passwords); per-app role model (none/webUI-low/webUI-med/webUI-full/full); JWT machine tokens for API/MCP; `X-Latarnia-App-Role` header injection; `latarnia_platform_{env}` Postgres DB for auth state. |
