...
- The dashboard "recent activity" feed shows every App's events to every user,
  leaking activity for Apps the user has no role on.
...
- **cap-005 — Activity feed role-filtered.** `GET /api/activity/recent` returns,
  for a non-Superuser, only events whose source App the user holds the `full` role
  on; system/non-App events are hidden. A Superuser sees all events.
- **cap-006 — Hard-delete a user.** A Superuser can permanently delete a user.
  ...
  (cascade); rows where the deleted user is `granted_by` are preserved with
  `granted_by` set to NULL (migration 006).
- **cap-007 — Re-activate a user.** A Superuser can re-activate a deactivated user
  who still holds valid credentials, without re-enrollment.
- **cap-008 — Re-issue TOTP setup.** A Superuser can re-issue a user's authenticator
  enrollment link. This deactivates the user and kills their sessions, deletes
  their existing TOTP credential (so a fresh secret is generated), and mints a new
  single-use `setup_token`. The user re-enrolls at `/auth/setup?token=…`.
...
flowchart TD
    ...
    C -- No --> E[For each event: resolve source app_id -> App name]
    E --> F{User role for that App == full?}
    F -- Yes --> G[Keep event]
    F -- No / system event --> H[Drop event]
...
- **High-level approach:** small, additive changes to existing modules. A new
  ...
  `main.py`; UI changes are conditionals in `dashboard.html` driven by the
  current user's `is_superuser` and per-App roles.
- **Identity on `/api/*`:** handlers must resolve the principal and its Superuser
  status. For session callers use `resolve_session_user`; for Bearer callers use
  the `super` JWT claim. A shared helper (e.g. `require_superuser(request)`)
  should centralize the 403.
...
- **Activity source mapping (cap-005):** events carry `source = app_id`; App
  roles key on `app_name` (the `latarnia.json` name). Resolve `app_id → name` via
  the App Registry when filtering. Events with `source ∈ {system, service_manager}`
  are App-less → Superuser-only.
- **Dashboard role context:** the dashboard is served at `/` behind `forward_auth`,
  which injects `X-Latarnia-Is-Super`. Render-time conditionals can read the
  current user server-side (the dashboard route resolves the session user) — do
  not rely on client JS alone for hiding, since cap-003/004 also enforce server
...
- Do NOT change `/mcp` auth or the machine-token issuance flow.
